...
public class ContaDAO {
	...
	public void adiciona(Conta conta) {
		String sql = "insert into contas (id_conta, descricao, paga, valor, tipo) values (nextval('contas_idConta_seq'),?,?,?,?)";
	...
	public void altera(Conta conta) {
		String sql = "update contas set descricao = ?, paga = ?, dataPagamento = ?, tipo = ?, valor = ? where id_conta = ?";
		PreparedStatement stmt;
		try {
			stmt = connection.prepareStatement(sql);
			stmt.setString(1, conta.getDescricao());
	...
	public Conta buscaPorId(Long id) {
		...
		try {
			PreparedStatement stmt = this.connection
					.prepareStatement("select * from contas where id_conta = ?");
			stmt.setLong(1, id);
			ResultSet rs = stmt.executeQuery();
	...
	public void paga(Long id) {
		...
		String sql = "update contas set paga = ?, dataPagamento = ? where id_conta = ?";
		PreparedStatement stmt;
		try {
			stmt = connection.prepareStatement(sql);
